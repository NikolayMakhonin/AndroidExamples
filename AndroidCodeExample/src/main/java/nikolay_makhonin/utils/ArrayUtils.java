...
public class ArrayUtils {
    ...
    public static <T, U> T[] copyOf(final U[] original, final int allocateLength, final int usageLength,
        final Class<? extends T[]> newType)
    {
        final T[] copy = ((Object) newType == (Object) Object[].class)
            ? (T[]) new Object[allocateLength]
            : (T[]) Array.newInstance(newType.getComponentType(), allocateLength);
     ...
     * @throws NegativeArraySizeException if <tt>newLength</tt> is negative
     * @throws NullPointerException       if <tt>original</tt> is null
     * @throws ArrayStoreException        if an element copied from <tt>original</tt> is not of a runtime type that can be stored in an array
     ...
     * @param a   the array to be searched
     * @param key the value to be searched for
     * @param c   the comparator by which the array is ordered. A <tt>null</tt> value indicates that the elements'
     ...
     * <tt>a.length</tt> if all elements in the array are less than the specified key. Note that this guarantees
     * that the return value will be &gt;= 0 if and only if the key is found.
     * @throws ClassCastException if the array contains elements that are not <i>mutually comparable</i> using the specified
